...
/**
 * Validates a batch of websites and finds alternatives if needed
 * @param request The batch validation request (an array of validation requests)
 * @returns A tuple with the response and error
 */
export async function validateAndFindWebsiteBatch(
	request: DTO_ValidateAndFindWebsiteRequestBatch,
): Promise<[DTO_ValidateAndFindWebsiteResponseBatch | null, string | null]> {
	try {
		// Get the API URL and auth token from environment variables
		const apiUrl = process.env.SUPPORT_SERVICES_URL;
		const authToken = process.env.SUPPORT_SERVICES_AUTH_TOKEN;
		...
		// Prepare the request - send the array directly
		const response = await fetch(`${apiUrl}/validate_and_find_website_batch`, {
			method: 'POST',
			headers: {
				'Content-Type': 'application/json',
				Authorization: authToken,
		...
		const data = await response.json();
		return [data as DTO_ValidateAndFindWebsiteResponseBatch, null];
	} catch (error) {
		console.error('Error in validateAndFindWebsiteBatch:', error);
		return [null, error instanceof Error ? error.message : String(error)];
	}
}
